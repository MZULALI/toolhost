...
import { ToolWorkerClient } from "../src/worker/client.ts";
...
test("a worker that fails to load reports startup_failed and is not supervised", () =>
  withTempDir(async (dir) => {
    ...
    const restarting: unknown[] = [];
    client.on("restarting", (info: any) => restarting.push(info));
    await assert.rejects(client.restart("test"), (error: any) => /Worker failed to start/.test(error.message));
...
test("callTool before start rejects immediately, not after a timeout", () =>
  withTempDir(async (dir) => {
    const client = new ToolWorkerClient({ config: { dir, dbPath: path.join(dir, "t.sqlite"), modulesDir: path.join(dir, "m"), workspace: dir } });
    const started = Date.now();
    await assert.rejects(client.callTool("x", {}), (error: any) => error.code === "worker_unavailable");
...
test("a raw errno during worker startup arrives as startup_failed without the host path", () =>
  withTempDir(async (dir) => {
    ...
    await assert.rejects(
      client.restart("test"),
      (error: any) => error.code === "startup_failed" && !error.message.includes(dir) && typeof error.details.remoteCode === "string"
    );
    await client.stop();
  }));
